<?php
/**
 * Elementor SMS Sent Messages Class
 *
...
class FarazSMS_Next_Elementor_SMS_Sent {
    ...
    public static function table() {
        ...
        // Capability gate — destructive actions on stored SMS rows require admin rights.
        $can_manage = current_user_can( 'manage_options' );

        // Handle delete action
        if ($can_manage && isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['item'])) {
            check_admin_referer('farazsms_elementor_delete_sms');
            $item_id = absint($_GET['item']);
            $table_name = FarazSMS_Next_Elementor_SMS_SQL::sent_table();
        ...
        if ($can_manage && isset($_POST['bulk_action']) && $_POST['bulk_action'] === 'bulk_delete' && isset($_POST['item']) && is_array($_POST['item'])) {
            check_admin_referer('farazsms_elementor_sent_bulk_action');
            $table_name = FarazSMS_Next_Elementor_SMS_SQL::sent_table();
        ...
        $form_id_raw = isset($_GET['id']) ? wp_unslash($_GET['id']) : '';
        $form_id_filter = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key($form_id_raw);
        if ($form_id_filter === '' && $form_id_raw !== '' && ctype_digit((string) $form_id_raw)) {
            $form_id_filter = (string) absint($form_id_raw);
        }
        $sent_messages = FarazSMS_Next_Elementor_SMS_SQL::get_sent_messages($form_id_filter ?: null, null, 100);
        ?>
        ...
        <div class="wrap">
            ...
            <?php if ($form_id_filter) { ?>
                <a href="admin.php?page=elementor_farazsms&view=sent" class="button"><?php _e('نمایش همه', 'farazsms-next'); ?></a>
            <?php } ?>

            <form method="post" action="">
                <?php wp_nonce_field('farazsms_elementor_sent_bulk_action', 'farazsms_elementor_sent_bulk_action'); ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="row-actions">
                                        <span class="delete">
                                            <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(array('action' => 'delete', 'item' => $message['id'])), 'farazsms_elementor_delete_sms')); ?>">
                                                <?php _e('حذف', 'farazsms-next'); ?>
                                            </a>
                                        </span>
                                    </div>
                                </td>
                                <td><?php echo esc_html($message['form_id']); ?></td>
                                <td><?php echo esc_html($message['entry_id']); ?></td>
