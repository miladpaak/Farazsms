<?php
/**
 * Elementor SMS Configurations Class
 *
 * Handles feed configuration/editing page for Elementor forms
 */
...
class FarazSMS_Next_Elementor_SMS_Configurations {
    ...
    public static function register_ajax_handlers() {
        ...
        if (!has_action('wp_ajax_farazsms_elementor_select_form')) {
            add_action('wp_ajax_farazsms_elementor_select_form', array(__CLASS__, 'select_form_ajax'), 10);
        }
        if (!has_action('wp_ajax_farazsms_elementor_create_pattern')) {
            add_action('wp_ajax_farazsms_elementor_create_pattern', array(__CLASS__, 'create_pattern_ajax'), 10);
        }
        if (!has_action('wp_ajax_farazsms_elementor_get_pattern')) {
            add_action('wp_ajax_farazsms_elementor_get_pattern', array(__CLASS__, 'get_pattern_ajax'), 10);
        }
    }
    ...
    public static function configuration() {
        ...
        wp_print_styles(array('jquery-ui-styles', 'wp-pointer', 'dashicons')); ?>
        ...
        <div class="wrap gforms_edit_form">
            ...
            $config = empty($id) ? array(
                'is_active' => true,
                'meta' => array()
            ) : FarazSMS_Next_Elementor_SMS_SQL::get_feed($id);

            $fid_param = function_exists('rgget') ? rgget('fid') : (isset($_GET['fid']) ? wp_unslash($_GET['fid']) : '');
            $gf_fid_param = function_exists('rgget') ? rgget('gf_fid') : (isset($_GET['gf_fid']) ? wp_unslash($_GET['gf_fid']) : '');
            $fid_san = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key($fid_param);
            $gf_fid_san = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key($gf_fid_param);
            $_get_form_id = !empty($config['form_id'])
                ? FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key($config['form_id'])
                : ($fid_san ?: $gf_fid_san);
            ...
            if (!empty($_POST['farazsms_feed_submit'])) {
                ...
                $config['form_id'] = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key(isset($_POST['gf_farazsms_form']) ? wp_unslash($_POST['gf_farazsms_form']) : '');
                ...
                $id = FarazSMS_Next_Elementor_SMS_SQL::update_feed($id, $config['form_id'], $config['is_active'], $config['meta']);
                ...
                // Redirect to Elementor submenu
                wp_redirect(admin_url('admin.php?page=elementor_farazsms&view=edit&id=' . $id . '&updated=true'));
                exit;
            }
            ...
            if (empty($_get_form_id)) {
                ...
                $fid_san = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key($fid_param);
                $gf_fid_san = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key($gf_fid_param);
                $_get_form_id = $fid_san ?: $gf_fid_san;
            }

            $updated = isset($_GET['updated']) ? sanitize_text_field($_GET['updated']) : '';
            if ($updated == 'true') {
                $back_url = admin_url('admin.php?page=elementor_farazsms');
            ...
            <form method="post" action="" id="gform_form_settings">
                ...
                <table class="gforms_form_settings" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="gf_farazsms_form" name="gf_farazsms_form" onchange="SelectFormAjax(jQuery(this).val());">
                                ...
                                <?php
                                // Get Elementor forms (pages/posts with form widgets)
                                $forms = self::get_elementor_forms();
                                foreach ((array)$forms as $form) {
                                    $selected = (string) $form['id'] === (string) $_get_form_id ? "selected='selected'" : "";
                                    ?>
                    ...
                    <tr id="gf_farazsms_field_group4" <?php echo empty($_get_form_id) ? "style='display:none;'" : ""; ?>>
                        ...
                        <td>
                            <?php
                            // Always show merge tags dropdown
                            $form_fields = array();
                            if (!empty($_get_form_id)) {
                                // Get Elementor form fields
                                $form_fields = self::get_elementor_form_fields($_get_form_id);
                            }
                            ?>
                            <select id="gf_farazsms_message_variable_select" onchange="InsertVariable('gf_farazsms_message');">
                                <?php echo self::get_elementor_form_fields_merge($form_fields); ?>
                            </select>
                            <br/>
                        ...
                        <td id="farazsms_customer_field_container">
                            ...
                            if (!empty($_get_form_id)) {
                                ...
                                if (!empty($form_fields)) {
                                    echo self::get_elementor_client_information($form_fields, $config);
                                } else {
                    ...
                    <tr id="gf_farazsms_field_group9" <?php echo empty($_get_form_id) ? "style='display:none;'" : ""; ?>>
                        <th><?php _e('زمان ارسال', 'farazsms-next'); ?></th>
                        <td>
                    ...
                    <tr id="gf_farazsms_field_group10" <?php echo empty($_get_form_id) ? "style='display:none;'" : ""; ?>>
                        <td colspan="2" style="padding-top:20px;">
                            ...
                            <?php
                            $cancel_url = admin_url('admin.php?page=elementor_farazsms');
                            ?>
        ...
        <script type="text/javascript">
            ...
            function SelectFormAjax(formId) {
                ...
                // Make AJAX request
                jQuery.post(ajaxurl, {
                    action: 'farazsms_elementor_select_form',
                    farazsms_elementor_select_form: '<?php echo wp_create_nonce('farazsms_elementor_select_form'); ?>',
                    form_id: formId
                }, function(response) {
                    if (response.success) {
            ...
            jQuery(document).ready(function($) {
                ...
                $('#create_pattern_admin_btn').on('click', function() {
                    ...
                    var formId = jQuery('#gf_farazsms_form').val() || 0;
                    jQuery.post(ajaxurl, {
                        action: 'farazsms_elementor_create_pattern',
                        farazsms_elementor_create_pattern: '<?php echo wp_create_nonce('farazsms_elementor_create_pattern'); ?>',
                        message: messageText,
                        type: 'admin',
                        feed_id: feedId,
                        form_id: formId
                        ...
                        if (response.success) {
                            if (response.data.feed_id) {
                                jQuery('input[name="farazsms_setting_id"]').val(response.data.feed_id);
                            }
                ...
                window.displayPatternInfo = function(type, patternCode, patternDetails) {
                    ...
                    if (patternDetails && typeof patternDetails === 'object') {
                        ...
                        // Variables - Check vars first (API standard), then attributes as fallback
                        if (data.vars && Array.isArray(data.vars) && data.vars.length > 0) {
                ...
                function loadSavedPatterns() {
                    ...
                    if (adminCode) {
                        jQuery.post(ajaxurl, {
                            action: 'farazsms_elementor_get_pattern',
                            farazsms_elementor_get_pattern: '<?php echo wp_create_nonce('farazsms_elementor_get_pattern'); ?>',
        ...
        <script type="text/javascript">
            ...
            ?>
            var form = [];
            form = <?php echo !empty($form_fields) ? json_encode($form_fields) : '[]'; ?>;
            ...
            function GetSelectableFields(selectedFieldId, labelMaxCharacters) {
                ...
                if (form && Array.isArray(form)) {
                    ...
                    for (var i = 0; i < form.length; i++) {
                        fieldLabel = form[i].label || form[i].id;
                        fieldType = form[i].type || '';
                        if (IsConditionalLogicField(form[i])) {
                            var selected = form[i].id == selectedFieldId ? "selected='selected'" : "";
                            str += "<option value='" + form[i].id + "' " + selected + ">" + TruncateMiddle(fieldLabel, labelMaxCharacters) + "</option>";
                        }
                    }
                }
                return str;
            ...
            function IsConditionalLogicField(field) {
                ...
                var supported_fields = ["text", "textarea", "email", "tel", "url", "number", "select", "checkbox", "radio", "acceptance", "date", "time"];
                var index = jQuery.inArray(fieldType, supported_fields);
                return index >= 0;
            }
            ...
            function GetConditionalFieldValues(input, fieldId, selectedOperator, selectedValue, labelMaxCharacters, index) {
                ...
                if (selectedOperator == '' || selectedOperator == 'is' || selectedOperator == 'isnot') {
                    // Elementor form fields may have options for select/radio/checkbox
                    if (field.options && Array.isArray(field.options) && field.options.length > 0) {
                        str += "<select name='" + name + "' id='" + input + "_value' class='condition_field_value'>";
                        for (var i = 0; i < field.options.length; i++) {
                            var optionValue = field.options[i].value || field.options[i];
                            var optionText = field.options[i].label || field.options[i].text || field.options[i];
                            var selected = (optionValue == selectedValue || optionText == selectedValue) ? "selected='selected'" : "";
                            str += "<option value='" + optionValue + "' " + selected + ">" + optionText + "</option>";
                        }
                        str += "</select>";
                    } else {
                        is_text = true;
     ...
     * @return array Array of merge tags in order: array('{field:field_id}', '{field:field_id2}', ...)
     */
    private static function extract_merge_tags_mapping($message) {
        $merge_tags = array();
        // Pattern to match all Elementor merge tags: {field:field_id}, {form_title}, {form_id}, etc.
        preg_match_all('/{([^}]+)}/i', $message, $matches, PREG_SET_ORDER);
        
        foreach ($matches as $match) {
            $merge_tags[] = $match[0]; // Full merge tag: {field:field_id}, {form_title}, {form_id}, etc.
        }
    ...
    /**
     * Convert Elementor merge tags to pattern format (%var%, %var2%, etc.)
     *
     * @param string $message Message text with Elementor merge tags
    ...
    public static function create_pattern_ajax() {
        // Check nonce
        $nonce = isset($_REQUEST['farazsms_elementor_create_pattern']) ? $_REQUEST['farazsms_elementor_create_pattern'] : '';
        if (empty($nonce) || !wp_verify_nonce($nonce, 'farazsms_elementor_create_pattern')) {
        ...
        $form_id = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key(isset($_REQUEST['form_id']) ? wp_unslash($_REQUEST['form_id']) : '');
        ...
        $description = __('پترن ساخته شده از Elementor', 'farazsms-next') . ' - ' . ($type === 'admin' ? __('پیام مدیر', 'farazsms-next') : __('پیام کاربر', 'farazsms-next'));
        $existing_code = '';
        if (!empty($feed_id) && $feed_id > 0) {
            $feed = FarazSMS_Next_Elementor_SMS_SQL::get_feed($feed_id);
        ...
        if ($result && isset($result['status']) && $result['status'] === 'success') {
            ...
            // Save pattern code and message to feed meta
            if (empty($feed_id) && !empty($form_id)) {
                // Create a new feed if we don't have one yet
                $feed_id = FarazSMS_Next_Elementor_SMS_SQL::update_feed(0, $form_id, 1, array());
            }
            ...
            if (!empty($feed_id) && $feed_id > 0) {
                ...
                if ($feed) {
                    ...
                    FarazSMS_Next_Elementor_SMS_SQL::update_feed($feed_id, $feed['form_id'], $feed['is_active'], $feed['meta']);
                }
            }
    ...
    public static function get_pattern_ajax() {
        // Check nonce
        $nonce = isset($_REQUEST['farazsms_elementor_get_pattern']) ? $_REQUEST['farazsms_elementor_get_pattern'] : '';
        if (empty($nonce) || !wp_verify_nonce($nonce, 'farazsms_elementor_get_pattern')) {
            wp_send_json_error(array('message' => __('خطای امنیتی', 'farazsms-next')));
            return;
        }
    ...
    public static function select_form_ajax() {
        // Check nonce
        $nonce = isset($_REQUEST['farazsms_elementor_select_form']) ? $_REQUEST['farazsms_elementor_select_form'] : '';
        if (empty($nonce) || !wp_verify_nonce($nonce, 'farazsms_elementor_select_form')) {
            wp_send_json_error(array('message' => __('خطای امنیتی', 'farazsms-next')));
            return;
        }
        ...
        // Get Elementor form fields
        $form_fields = self::get_elementor_form_fields($form_id);
        
        if (empty($form_fields)) {
            wp_send_json_error(array('message' => __('فرم یافت نشد یا فیلدی ندارد', 'farazsms-next')));
            return;
        }
        
        // Get fields for merge tags
        $fields_html = self::get_elementor_form_fields_merge($form_fields);
        
        // Get customer phone field
        $customer_field_html = self::get_elementor_client_information($form_fields, array());
    ...
    /**
     * Parse storage key into post ID and optional Elementor widget id.
     *
     * @param string $form_key Sanitized or raw key.
     * @return array{0:int,1:string|null} post_id, widget_id or null for legacy numeric-only key.
     */
    private static function parse_elementor_form_storage_key($form_key) {
        $k = FarazSMS_Next_Elementor_SMS_SQL::sanitize_form_storage_key($form_key);
        if ($k === '') {
            return array(0, null);
        }
        if (strpos($k, '_') !== false) {
            $parts = explode('_', $k, 2);
            return array(absint($parts[0]), isset($parts[1]) ? sanitize_key($parts[1]) : null);
        }

        return array(absint($k), null);
    }

    /**
     * Build field rows from one Elementor form widget settings.
     *
     * @param array $widget Element model (must be widgetType form).
     * @return array<int, array<string, mixed>>
     */
    private static function collect_fields_from_elementor_form_widget($widget) {
        $fields = array();
        if (empty($widget['settings']['form_fields']) || !is_array($widget['settings']['form_fields'])) {
            return $fields;
        }
        foreach ($widget['settings']['form_fields'] as $field) {
            $field_id = isset($field['custom_id']) ? $field['custom_id'] : (isset($field['_id']) ? $field['_id'] : '');
            $field_label = isset($field['field_label']) ? $field['field_label'] : (isset($field['label']) ? $field['label'] : '');
            $field_type = isset($field['field_type']) ? $field['field_type'] : '';

            if (empty($field_id)) {
                $field_id = isset($field['_id']) ? $field['_id'] : '';
            }
            ...
            if (!empty($field_id)) {
                $fields[] = array(
                    ...
                    'options' => isset($field['field_options']) ? $field['field_options'] : (isset($field['options']) ? $field['options'] : array()),
                );
            }
        }
    ...
    /**
     * Get Elementor forms (pages/posts/templates with form widgets)
     */
    private static function get_elementor_forms() {
        $forms = array();
        ...
        $post_types = apply_filters(
            'farazsms_next_elementor_form_post_types',
        ...
        foreach ($posts as $post) {
            ...
            $widgets = self::find_form_widgets($data);
            foreach ($widgets as $widget) {
                ...
                if ($post->post_type === 'elementor_library') {
                    $tt = get_post_meta($post->ID, '_elementor_template_type', true);
                    if (!empty($tt)) {
                        $label = '[' . $tt . '] ' . $label;
                    } else {
    ...
    /**
     * Check if Elementor data has form widget
     */
    private static function has_form_widget($data) {
        if (!is_array($data)) {
            return false;
        }
        ...
        foreach ($data as $element) {
            ...
            if (isset($element['elements']) && is_array($element['elements'])) {
                if (self::has_form_widget($element['elements'])) {
                    return true;
                }
            }
        }
    ...
    /**
     * Get Elementor form fields from a storage key (post or post_widget).
     *
     * @param string $form_key Composite or legacy post id string.
     * @return array<int, array<string, mixed>>
     */
    private static function get_elementor_form_fields($form_key) {
        $fields = array();
        ...
        list($post_id, $widget_id) = self::parse_elementor_form_storage_key($form_key);
        if (!$post_id) {
            return $fields;
        }
        ...
        $form_widgets = self::find_form_widgets($data);

        if ($widget_id) {
            foreach ($form_widgets as $widget) {
                $wid = isset($widget['id']) ? sanitize_key($widget['id']) : '';
                if ($wid && $wid === $widget_id) {
                    return self::collect_fields_from_elementor_form_widget($widget);
                }
            }
        ...
        foreach ($form_widgets as $widget) {
            $fields = array_merge($fields, self::collect_fields_from_elementor_form_widget($widget));
        }
    ...
    /**
     * Find form widgets in Elementor data
     */
    private static function find_form_widgets($data, $widgets = array()) {
        if (!is_array($data)) {
            return $widgets;
        }
        ...
        foreach ($data as $element) {
            ...
            if (isset($element['elements']) && is_array($element['elements'])) {
                $widgets = self::find_form_widgets($element['elements'], $widgets);
            }
        }
    ...
    /**
     * Get Elementor form fields for merge tags
     */
    private static function get_elementor_form_fields_merge($form_fields) {
        ...
        if (!empty($form_fields) && is_array($form_fields)) {
            ...
            if (!empty($required_fields)) {
                ...
                foreach ($required_fields as $field) {
                    ...
                    $field_label = isset($field['label']) ? $field['label'] : $field_id;
                    if (!empty($field_id)) {
                        $str .= '<option value="{field:' . esc_attr($field_id) . '}">' . esc_html($field_label) . '</option>';
                    }
                }
                $str .= '</optgroup>';
            }
        ...
        // Other options (always show, even if no form fields)
    ...
    /**
     * Get Elementor client phone field select (shows all fields)
     */
    private static function get_elementor_client_information($form_fields, $config) {
        ...
        foreach ((array)$form_fields as $field) {
            $field_id = isset($field['id']) ? $field['id'] : '';
            $field_label = isset($field['label']) ? esc_html($field['label']) : $field_id;
